 ...
 * version 2.1 of the License, or (at your option) any later version.
    FFmpeg是自由软件;你可以和/或重新分配修改它根据GNU Lesser General公开许可证是由自由软件基金会发布;2.1版本的许可,或(任您选)其后的版本。
 *
 * FFmpeg is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
 * Lesser General Public License for more details.
 FFmpeg是分布式的,希望这将是有用的,但是没有任何保证;没有即使的默示保证适销性或健身为特定目的。看到GNU较小的通用公共许可证的更多细节。
 
 * You should have received a copy of the GNU Lesser General Public
 * License along with FFmpeg; if not, write to the Free Software
 * Foundation, Inc., 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA
 你应该已经收到了关于FFmpeg的GNU Lesser General公共的副本许可证;如果没有,写免费的软件框架,Inc .,51富兰克林街,五楼,波士顿02110 - 1301美国
 */
 ...
 * follows:
    编码/解码,音频/视频的API是非常类似的,其工作如下:
 * - Set up and open the AVCodecContext as usual.   
    通常设置和打开AVCodecContext。
 * - Send valid input:                              
    发送有效的输入
 *   - For decoding, call avcodec_send_packet() to give the decoder raw
 *     compressed data in an AVPacket.             
    解码,调用avcodec_send_packet()给解码器生在一个AVPacket压缩数据
 *   - For encoding, call avcodec_send_frame() to give the encoder an AVFrame
 *     containing uncompressed audio or video.
    编码,调用avcodec_send_frame()给编码器一个包含未压缩的音频或视频的AVFrame
 *   In both cases, it is recommended that AVPackets and AVFrames are
 *   refcounted, or libavcodec might have to copy the input data. (libavformat
 *   always returns refcounted AVPackets, and av_frame_get_buffer() allocates
 *   refcounted AVFrames.)
    在这两种情况下,建议AVPackets和AVFramesrefcounted是被引用的，或libavcodec拥有一份输入数据的拷贝。(libavformat总是返回引用的 AVPackets,av_frame_get_buffer()分配引用着的 AVFrames)。
 ...
 *
    接收输出在一个循环中。定期调用avcodec_receive_*()函数和过程输出:
    解码,调用avcodec_receive_frame()。如果成功,它将返回一个包含未压缩的音频或视频数据的AVFrame。
    编码,调用avcodec_receive_packet()。如果成功,它将返回一个AVPacket压缩帧.
    重复这个调用,直到它返回AVERROR(EAGAIN)或一个错误。AVERROR(EAGAIN)返回值意味着返回新的输出必须需要新的输入数据。在这种情况下,继续发送输入数据。对于每一个输入的帧/包,编解码器通常会返回1个输出帧/包,但它也可以是0比1或更多。

 * At the beginning of decoding or encoding, the codec might accept multiple
 * input frames/packets without returning a frame, until its internal buffers
 * are filled. This situation is handled transparently if you follow the steps
 * outlined above.
    在刚开始解码和编码时,解码器可能接受多个输入帧/数据包而没有返回一个帧,直到其内部缓冲区填满为止。如果你遵循上面列出的这些步骤这种情况是非常显然的。
 *
 ...
 * returns EAGAIN.
    理论上,发送输入会导致EAGAIN——这应该只有没有接收到任何输出是发生。您可以使用此结构替代解码或其他编码循环比上面的建议。例如,假如返回EAGAIN时，您可以试着在每一次迭代时发送新的输入，并试着接收到输出。
 ...
 *   avcodec_flush_buffers().
    流在结束的情况下。这里需要“冲洗”(又名排水)编码,出于性能或者必要时编解码器可能缓冲多个帧或包(考虑B-frames)。这时处理如下:
    把NULL输入avcodec_send_packet()(解码)或avcodec_send_frame()函数(编码)来替换可用的输入。这将进入draining模式。
    调用avcodec_receive_frame()(解码)或avcodec_receive_packet()(编码)在一个循环,直到返回AVERROR_EOF。这个函数将不返回AVERROR(EAGAIN),除非你忘了进入draining模式。
    解码恢复之前,编解码器必须重置avcodec_flush_buffers()。
 *
 ...
 * read at least some output.
    强烈推荐使用上面列出的API。但也可以使用rigid schema模式以外的函数。例如,您可以反复调用avcodec_send_packet()在没有调用avcodec_receive_frame()时。在这种情况下,avcodec_send_packet()直到编解码器的内部缓冲区被填满才会成功(这是通常的每个输出帧大小1,初始输入后),然后拒绝输入AVERROR(EAGAIN)。一旦它开始拒绝输入,你别无选择但可用读一些输出。
 *
 ...
 * permit unlimited buffering of input or output.
    并不是所有的编解码器将遵循严格的和可预测的数据流;他唯一的保证是一个AVERROR(EAGAIN)返回值在电话发送/接收一端时将意味着接收/发送电话另一端会成功,或者至少不会失败返回AVERROR(EAGAIN)。一般来说,没有编解码器允许无限缓冲的输入或输出。
 *
 ...
 * - The new API does not handle subtitles yet.
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    这个API取代以下旧的功能:
    ——avcodec_decode_video2()和avcodec_decode_audio4():
    使用avcodec_send_packet()来输入数据到解码器,然后使用avcodec_receive_frame()来接收每个包解码后的数据帧。
    与旧的视频解码API相比,多个帧可能来自同一个包。对音频来说,对于API使用者来说，把输入的数据包分开放置帧数据包里面。你永远不需要第二次给一个AVPacket到这个API(除非它被拒绝两次返回AVERROR(EAGAIN)没有从packet中读取到数据)。此外,发送一个flush冲洗/draining包只需要一次。
    ——avcodec_encode_video2()/ avcodec_encode_audio2():
    使用avcodec_send_frame()来输入数据到编码器,然后使用avcodec_receive_packet()接收编码后的数据包。avcodec_receive_packet()不会为提供用户缓存区。
    ——新的API不处理字幕。
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 * @}
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    在相同的AVCodecContext中不允许同时混合使用新的和旧的函数，会导致未定义的行为。
    一些编码可能需要使用新的API，使用旧的API将会返回一个错误。新的API支持所有的编码。
    一个编解码器是不允许发送和接收都返回返回AVERROR（EAGAIN）。这将是一个无效的状态，这将会把编码器带入到一个无尽的循环。这个API没有时间概念：这样的不会发生：通过avcodec_send_packet()来产生AVERROR(EAGAIN)，但是他会一秒后再次调用该函数来接收这个packet（不会涉及其他接收/刷新的API的调用）。这个API是一个严格的机器状态，时间的流逝不会对他产生影响。在一些情况下，一些可靠的行为仍然是可以接受的。但是他必须保证不会同时在发送/接收时返回AVERROR（EAGAIN）。
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 * @{
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 @defgroup lavc_core核心功能、结构体.
 * @ingroup libavc
    *基本定义、函数查询libavcodec功能,*分配核心结构等。
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 * descriptor list and bump libavcodec minor version.
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    确定比特流的语法和语义。
    大致的原则是:
    两个有相同的ID的解码器可以解码相同的流。
    两个有相同的ID的编码器可以编码兼容的流。
    在处理细节的原则上可能有轻微的偏离。
    如果你添加一个编码器ID到这个列表中，加入他以便
    1、不会导致一个已经存在的值的编解码ID发生变化（这会导致ABI（接口）的崩溃）
    2、对于相似的编解码器是非常可能的。
    添加新的编解码器id之后,不要忘记将一个条目添加到编解码器描述符列表和添加到libavcodec minor版本中。
 ...
 * @see avcodec_descriptor_get()
    ...
    这个结构体通过一个AVCodecID描述了一个编解码器
    看avcodec_descriptor_get()方法 
    6363行
 */
typedef struct AVCodecDescriptor {
     ...
     * characters and '_' only.
     描述编解码器的名字。对每一个编解码器来说是一个不为空的并且唯一的。它应该只包含字母数字字符和“_”。
     */
    const char      *name;
    /**
     * A more descriptive name for this codec. May be NULL.
    一个更能够描述这个编解码器的名字。可能为NULL
     */
    const char *long_name;
    /**
     * Codec properties, a combination of AV_CODEC_PROP_* flags.
     编解码器的属性，一个和AV_CODEC_PROP_*的组合的标志
     */
    int             props;
    /**
     ...
     * The first item is always non-NULL and is the preferred MIME type.
     多种用途的类型。可能为NULL。假如不是NULL，则他是一个以NULL为结束的MIME类型的数组。
     第一项总是非NULL，并且是首先的MIME类型。
     */
    const char *const *mime_types;
    /**
     * If non-NULL, an array of profiles recognized for this codec.
     * Terminated with FF_PROFILE_UNKNOWN.
     假如不是为NULL，则是一个关于这个编解码器的配置组织的数组。并且以FF_PROFILE_UNKNOWN作为结束。
     */
 ...
 * compression modes
    编解码器支持有损压缩，只有视频和音频解码器使用。
    注意，一个编解码器可能同时支持有损和无损压缩模式
 */
#define AV_CODEC_PROP_LOSSY         (1 << 1)
/**
 * Codec supports lossless compression. Audio and video codecs only.
    编解码器支持无损压缩，只有视频和音频解码器使用
 */
 ...
 * equal.
    编解码器支持帧重新排序。这就是说，这个编码顺序可能和帧顺序表现的不一样（这个解码的packets顺序是通过 encoders、stored、input来决定的）。
    对于编解码器来说，假如没有这个属性集，PTS和DTS应该总是一样的。
 */
#define AV_CODEC_PROP_REORDER       (1 << 3)
/**
 * Subtitle codec is bitmap based
 * Decoded AVSubtitle data can be read from the AVSubtitleRect->pict field.
    副标题解码器是基于的位图的。解码的AVSubtitle数据可以从AVSubtitleRect的pict field中读取。
 */
 ...
 * Decoded AVSubtitle data can be read from the AVSubtitleRect->ass field.
    副标题解码器是基于文本的。
    解码的AVSubtitle数据可以从AVSubtitleRect的ass field中读取
 */
 ...
 * MPEG bitstreams could cause overread and segfault.
    对于解码需要额外分配的字节在输入比特流的末尾。
    这是主要因为有些需要优化的比特流阅读者一次读入32或者62个bit并且阅读出界。
    注意：如果第一个23位的额外的字节不是0，这个损坏了的MPEG比特流可能会导致越界和段错误。
 */
 ...
 * @deprecated use AV_INPUT_BUFFER_PADDING_SIZE instead
    使用AV_INPUT_BUFFER_PADDING_SIZE来进行替代
 */
#define FF_INPUT_BUFFER_PADDING_SIZE 32

/**
 * @deprecated use AV_INPUT_BUFFER_MIN_SIZE instead
    使用AV_INPUT_BUFFER_MIN_SIZE来进行替代
 */
#define FF_MIN_BUFFER_SIZE 16384
#endif /* FF_API_WITHOUT_PREFIX 没有使用FF作为前缀的API*/
...
enum Motion_Est_ID {
    ...
    ME_LOG,
    ME_PHODS,
    ME_EPZS,        ///< enhanced predictive zonal search   增强预测区域搜索
    ME_X1,          ///< reserved for experiments           用于实验
    ME_HEX,         ///< hexagon based search               基于六边形的搜索
    ME_UMH,         ///< uneven multi-hexagon search        基于不规则六边形的搜索
    ME_TESA,        ///< transformed exhaustive search algorithm    变换详尽的搜索算法
    ME_ITER=50,     ///< iterative search                   迭代搜索
};
#endif
...
enum AVDiscard{//丢弃
     ...
     * keyframes for intra-only or drop just some bidir frames).
     我们留下一些空间用于拓展（在内部放弃一些关键帧或者仅仅放弃一些双向帧）
     */
    AVDISCARD_NONE    =-16, ///< discard nothing                                        没有丢弃
    AVDISCARD_DEFAULT =  0, ///< discard useless packets like 0 size packets in avi     丢弃无用的数据包，比如在avi中0大小的数据包
    AVDISCARD_NONREF  =  8, ///< discard all non reference                              丢弃所有的不是参考的
    AVDISCARD_BIDIR   = 16, ///< discard all bidirectional frames                       丢弃所有的双向帧
    AVDISCARD_NONINTRA= 24, ///< discard all non intra frames                           丢弃所有的非内部帧
    AVDISCARD_NONKEY  = 32, ///< discard all frames except keyframes                    丢弃所有的帧，除了关键帧
    AVDISCARD_ALL     = 48, ///< discard all                                            丢弃所有的帧
};

enum AVAudioServiceType {//AV音频服务类型
    AV_AUDIO_SERVICE_TYPE_MAIN              = 0,        //主类型
    AV_AUDIO_SERVICE_TYPE_EFFECTS           = 1,        //影响的类型
    AV_AUDIO_SERVICE_TYPE_VISUALLY_IMPAIRED = 2,        //视觉受损的类型
    AV_AUDIO_SERVICE_TYPE_HEARING_IMPAIRED  = 3,        //听力受损的类型
    AV_AUDIO_SERVICE_TYPE_DIALOGUE          = 4,        //对话类型
    AV_AUDIO_SERVICE_TYPE_COMMENTARY        = 5,        //评论类型
    AV_AUDIO_SERVICE_TYPE_EMERGENCY         = 6,        //紧急类型
    AV_AUDIO_SERVICE_TYPE_VOICE_OVER        = 7,        //语言类型
    AV_AUDIO_SERVICE_TYPE_KARAOKE           = 8,        //卡拉OK类型
    AV_AUDIO_SERVICE_TYPE_NB                   , ///< Not part of ABI 不包含
};
...
typedef struct RcOverride{
    int start_frame;
    int end_frame;
    int qscale; // If this is 0 then quality_factor will be used instead. 如果是0则使用quality_factor进行替代
 ...
 * @deprecated there is no libavcodec-wide limit on the number of B-frames
    这里没有libavcodec-wide来限制B-frames的数量
 */
...
/* encoding support
   ...
   Note: Not everything is supported yet.
    在初始化之前这些标志可以通过AVCodecContext.flags传入。
    注意：不是所有的都是支持的。
*/
 ...
 * to CPU requirements (e.g. due to cropping).
    允许解码器产生不一致的数据帧列表的给CPU的需求(例如由于裁剪)。
 */
 ...
 * Use internal 2pass ratecontrol in first pass mode.
    在第1次pass mode时使用内部的2pass ratecontrol 微控制
 */
#define AV_CODEC_FLAG_PASS1           (1 <<  9)
/**
 * Use internal 2pass ratecontrol in second pass mode.
    在第2次pass mode时使用内部的2pass ratecontrol 微控制
 */
 ...
 * Place global headers in extradata instead of every keyframe.
    放置全局headers在extradata中而不是在每个关键帧中
 */
 ...
 * Place global headers at every keyframe instead of in extradata.
    在每个关键帧设置全局的headers来替代在extradata中的headers
 */
 ...
 * Do not skip samples and export skip information as frame side data
    不要跳过跳过帧的一边的数据来跳过样本和出口
 */
#define AV_CODEC_FLAG2_SKIP_MANUAL    (1 << 29)
/**
 * Do not reset ASS ReadOrder field on flush (subtitles decoding)
    不要重置ASS ReadOrder 字段来刷新（副标题）
 */
 ...
 * assume the buffer was allocated by avcodec_default_get_buffer.
    编解码器使用get_buffer() 来分配缓冲并支持自定义分配。假如没有设置，他可能不会使用get_buffer() 或者使用操作：假设缓冲是通过avcodec_default_get_buffer进行分配的。
 */
 ...
 *       be determined by libavcodec from the input frame.
    编码器或者解码器需要在末尾有NULL输入的flushing，以便于给完整和正确的输出。
    注意：假如没有设置这个标志，这个编解码器保证从来不给出NULL数据。用户仍然可以对公共的编码、解码函数发送NULL数据，但是libavcodec不会传递给编解码器，除非设置了这个标志。
    解码器：
    这个解码器有一个非零延迟，并且需要给avpkt->data=NULL,avpkt->size=0在结束位置来得到延迟数据，知道解码器不再返回帧。
    编码器：
    编码器需要在输入数据结束时使用NULL数据，直到编码器不在返回数据为止。
    注意：给编码器实现AVCodec.encode2()函数，设置这个标志也就意味着这个编码器必须为每个数据包设置pts和持续时间。假如这个标志没有设置，这个pts和持续时间将会由libavcodec从输入的帧来确定。
 */
#define AV_CODEC_CAP_DELAY               (1 <<  5)
/**
 * Codec can be fed a final frame with a smaller size.
 * This can be used to prevent truncation of the last audio samples.
    编解码器被给的最后一帧可以是较小尺寸。这可以被用来阻止最后的音频样本被删节。
 */
 ...
 * as a last resort.
    解码器可以从每一个AVPacket输出多个帧
    一般分配器在一个时间上只返回一个帧，分配器是不会做解析器来返回到的正确的帧中。这个flag是罕见的保留为编解码器操作的，不能把bit流分成帧的没有耗时的全解码操作。分配器携带这样的bit流可能会返回多帧来自同一个packet。这有许多缺点，比如在许多情况下禁止流的复制。所以他是只应该被考虑作为最后的手段使用。
 */
#define AV_CODEC_CAP_SUBFRAMES           (1 <<  8)
/**
 * Codec is experimental and is thus avoided in favor of non experimental
 * encoders
    编解码器是实验的，从而避免了非实验的编码
 */
#define AV_CODEC_CAP_EXPERIMENTAL        (1 <<  9)
/**
 * Codec should fill in channel configuration and samplerate instead of container
    编解码器应该填写channel配置和采样率来替代container（容器）
 */
 ...
 * Codec supports avctx->thread_count == 0 (auto).
    编解码器支持avctx->thread_count == 0（自动）
 */
#define AV_CODEC_CAP_AUTO_THREADS        (1 << 15)
/**
 * Audio encoder supports receiving a different number of samples in each call.
    音频解码器支持在每个调用中收到不同数量的样本。
 */
 ...
 * choice for probing.
    对于probing（探测）来说，解码器不是首选的。
    这表明解码器对于probing来说不是一个很好的选择。
    例如：是一个敖贵的硬件旋转解码器，或者不能从流中提供简单的用户信息。
    带有这个标志应的解码器应该只作为最后的手段。
 */
 ...
 * mpegvideo encoders
    在mpv_flags使用mpegvideo解码器的私有选项mv0
 */
 ...
 * mpegvideo encoders
    在mpv_flags使用mpegvideo解码器的私有选项naq
 */
...
#define CODEC_CAP_DRAW_HORIZ_BAND AV_CODEC_CAP_DRAW_HORIZ_BAND ///< Decoder can use draw_horiz_band callback. 解码器可以使用draw_horiz_band进行回调
/**
 * Codec uses get_buffer() for allocating buffers and supports custom allocators.
 * If not set, it might not use get_buffer() at all or use operations that
 * assume the buffer was allocated by avcodec_default_get_buffer.
 ...
 * chroma format, resolution etc.
    编解码器可以从HW编码中导出数据。这个flag表明这个编解码器可以调用get_format()的列表：这个列表可能包含HW加速的格式（XvMC，VDPAU，VAAPI等
    这个应用可以选择任何包含的原始图片数据。
    应用可以通过上下午来确定bit流版本，色度格式，分辨率等。
 */
 ...
 * codecs
    HWAccel是实验的，从而避免了非实验的编码
 */
#define HWACCEL_CODEC_CAP_EXPERIMENTAL     0x0200
#endif /* FF_API_WITHOUT_PREFIX */

#if FF_API_MB_TYPE
//The following defines may change, don't expect compatibility if you use them. 下面的定义可能会改变，不要期望他们的兼容性
...
// Note bits 24-31 are reserved for codec specific use (H.264 ref0, MPEG-1 0mv, ...)注意：24-31是留给编码器使用的
#endif
 ...
 * Note there may be multiple such areas for one frame.
    pan扫描区
    这个指定的区域应该被显示
    注意：对于一个帧来说可能有多个这样的区域
 */
typedef struct AVPanScan{
    /**
     * id
     * - encoding: Set by user. 编码时使用者来设置
     * - decoding: Set by libavcodec.解码时libavcodec来设置
     */
    int id;

    /**
     * width and height in 1/16 pel 1/16的像素的宽度和高度
     * - encoding: Set by user.     编码由使用者来设置
     * - decoding: Set by libavcodec.   解码有libavcodec来设置
     */
    int width;
    int height;

    /**
     * position of the top left corner in 1/16 pel for up to 3 fields/frames    3字段/帧的左上角 1/16像素的位置
     * - encoding: Set by user. 编码由使用者来设置
     * - decoding: Set by libavcodec.   解码有libavcodec来设置
     */
 ...
 * parameters for H.264/HEVC.
    这个结构描述了一个编码器的比特流的比特率属性。
    它大致对应于一个子集MPEG-2的VBV或H.264/HEVC的HRD属性。
 */
typedef struct AVCPBProperties {
     ...
     * UINT64_MAX when unknown or unspecified.
     packet被接收到和被解码两者之间的延迟，27MHz的时钟周期
     如果未知或者没有指明则为UINT64_MAX
     */
    uint64_t vbv_delay;
} AVCPBProperties;
...
/**
 * The decoder will keep a reference to the frame and may reuse it later.   解码器可能会保持一个帧的引用，并且可以重新用到它。
 */
...
enum AVPacketSideDataType {
     ...
     * present.
     一个AV_PKT_DATA_PALETTE里面的数据包包含确切的AVPALETTE_SIZE字节的调色板数据。这里面是一个新的调色板数据信号。
     */
    AV_PKT_DATA_PALETTE,
     ...
     * frame or packet.
     这个AV_PKT_DATA_NEW_EXTRADATA被用于通知编解码器或者这个格式，这个extradata缓存区发生了变化，并且接收通知的一方应该做出适当的行动。新的exradata嵌入到数据缓存区，并且应该马上处理当前的帧或者包。
     */
    AV_PKT_DATA_NEW_EXTRADATA,

    /**
     * An AV_PKT_DATA_PARAM_CHANGE side data packet is laid out as follows:
    一个AV_PKT_DATA_PARAM_CHANGE数据包布局如下:
     * @code
     * u32le param_flags
...
typedef struct AVCodec {
     ...
     * This is the primary way to find a codec from the user perspective.
     编解码器的名称
     这个名字在解码器和编码器之间是独一无二的，但是编码器和解码器是可以使用同一个名字，
     从用户角度看，根据名字找到编码器是最有用的方法。
     */
    const char *name;
    /**
     * Descriptive name for the codec, meant to be more human readable than name.
     * You should use the NULL_IF_CONFIG_SMALL() macro to define it.
     编解码器的描述性名称,应该是人类可读的多的名字.你可以使用NULL_IF_CONFIG_SMALL()来定义它
     */
    const char *long_name;
    enum AVMediaType type;
    enum AVCodecID id;
    /**
     * Codec capabilities.  //功能
     * see AV_CODEC_CAP_*
     */
    int capabilities;   //功能编号
    ...
    这条线下面的字段不是是属于公共API的一部分。他们可能不能被在libavcodec使用，不能改变或者删除。新的公共字段应该被添加在这个上面
     *****************************************************************
     */
    int priv_data_size;
    struct AVCodec *next;
 ...
 * instead.
 关闭一个给定的AVCodecContext和他关联的所有数据（但是不包括AVCodecContext他自己）
 调用这个函数在一个没有被打开的AVCodecContext时，将会释放编解码器通过avcodec_alloc_context3()分配的特定的数据为一个non_NULL编解码器，后续调用将什么都不做。
 note：不要使用这个函数。使用avcodec_free_context()来摧毁一个codec context（无论是打开还是关闭）。不支持多次打开和关闭一个codec context。需要使用多个codec context来替代。
 */
 ...
 * @deprecated Use avcodec_send_packet() and avcodec_receive_frame().
 ...
 解码视频的帧的大小avpkt->size 是 avpkt->data转换为图片的大小。一些解码器可能支持在一个AVPacket中包含多个帧的AVPacket，这些解码器则只会解码编码中的第一个帧；
 warning；输入缓存区必须AV_INPUT_BUFFER_PADDING_SIZE 大于实际读取的大小，因为有些优化的比特流阅读者会阅读32或者64bits，可能会超过阅读的界限。
 warning：混出去的最后的buf应该设置为0，保证不会发生越界阅读二损坏MPEG流。
 note：编解码器可以有AV_CODEC_CAP_DELAY功能来设置一个在输入和输出之间的延迟，这需要 avpkt->data=NULL，avpkt->size=0 来在结束时返回属于的帧。
 note：AVCodecContext在给packets解码钱必须是通过avcodec_open2()打开的。
 avctx参数：编解码器上下文
 [out]picture参数：在被解码的视频帧中将会存储AVFrame。使用av_frame_alloc()来得到一个AVFrame。这编解码器将会通过调用AVCodecContext.get_buffer2()为实际的bitmap申请内存，当AVCodecContext.refcounted_frames设置为1时，帧是引用计数器并且返回引用给调用者。当帧在长时间内不会被使用时调用者必须使用av_frame_unref()来是否这个帧。假如av_frame_is_writable()返回为1则调用者可以安全的写帧操作。当AVCodecContext.refcounted_frames设置为0时，将会返回这个编解码器的引用计数器直到下一次调用这个方法或者关闭或者刷新解码器。这样调用时不可已写他的。
 [in]avpkt参数：输入的AVPacket包含输入的缓存区。你可以通过av_init_packet()创造这样的包，任何设置数据和大小，一些解码器可能此外还需要其他一些字段，比如flags&AV_PKT_FLAG_KEY。所有的编码器是被设计尽可能的使用少的字段。
 [in,out] got_picture_ptr参数：假如没有解压出帧，就是0，否则就不会是0
 return：错误就是一个负值被返回，否则返回被使用的大小字节或者当没有解压出帧时返回0
 过时的：使用avcodec_send_packet() and avcodec_receive_frame().
 */
attribute_deprecated
